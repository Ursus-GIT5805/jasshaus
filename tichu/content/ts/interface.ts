...
import { Game, Card, parse_trick, Trick, Cardset, parse_all_tricks, contains_bomb } from "../pkg/tichu_game.js";
import { ClientData, ClientID, PlayerID } from "./wshandler.js";
import { CirclePlayer } from "./players.js";

declare global {
	interface JQuery { display(v: boolean): JQuery; }
}
...
export class UI {
	...
	setupPlayPass(
		onplay: (trick: Trick, wish: undefined | number) => void,
		onpass: () => void,
	) {
		$("#play").click(() => {
	...
	setupExchangeWindow(
		exchangeCallback: (cards: Card[]) => void,
	) {
