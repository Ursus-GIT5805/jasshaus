...
#[derive(Tsify)]
#[tsify(into_wasm_abi, from_wasm_abi)]
#[derive(Clone, PartialEq, Error, Debug, Serialize, Deserialize)]
...
impl Game {
	pub fn new(setting: Setting) -> Self {
		let players = (0..setting.num_players)
			.map(|i| Player::new(i & 1))
			.collect();
	...
	pub fn should_start_exchange(&self) -> bool {
		self.phase == Phase::Distributing
			&& !self
				.players
				.iter()
				.any(|p| p.num_cards != self.cards_per_player())
	}

	/// Returns true when it's time to evaluate the exchange
	pub fn should_end_exchange(&self) -> bool {
		self.phase == Phase::Exchange && !self.players.iter().any(|p| p.exchange.is_empty())
	}
	...
	pub fn end_round(&mut self) {
		...
		if let Some(first_plr) = self.get_first_finished_player() {
			...
			let unfinished = self.players.iter_mut().filter(|plr| !plr.finished());
	...
	fn handle_finish(&mut self, plr_id: usize) {
		...
		let num_finished_teams = num_plrs
			.iter()
			.zip(finished_plrs.iter())
			.filter(|(&total, &finished)| total == finished)
			.count();

		let num_at_least_one = finished_plrs.iter().filter(|&x| *x != 0).count();
	...
	pub fn can_pass(&self, plr_id: usize) -> bool {
		if self.phase != Phase::Playing || self.current_player != plr_id {
			return false;
		}
	...
	fn handle_tichu(&mut self, plr_id: usize) {
		...
		for (id, plr) in self.players.iter().enumerate() {
			...
			let sign = if id == plr_id { 1 } else { -1 };
	...
	pub fn can_give_away(&self, target: usize, plr_id: usize) -> bool {
		...
		self.phase == Phase::GiveAway && self.current_player == plr_id && tid1 != tid2
	}
	...
	pub fn num_finished_players(&self) -> usize {
		self.players.iter().filter(|plr| plr.finished()).count()
	}
	...
	pub fn num_unfinished_teams(&self) -> usize {
		...
		finished.iter().filter(|&b| !b).count()
	}
	...
	pub fn should_game_end(&self) -> bool {
		match self.setting.end_condition {
			EndCondition::Points(points) => {
				...
				let num_best = self.teams.iter().filter(|team| team.points == mx).count();
