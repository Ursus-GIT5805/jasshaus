...
class VoteManager {
	...
	constructor(
		...
		let cont = $('<div class="VoteWindow">').append($("<h2>").text(text)).append(ele);
		this.ele = cont;
...
export class VoteHandler {
	...
	constructor(container?: JQuery<HTMLElement>, callback?: (index: number) => void) {
	...
	onnewvote(type: string) {
		...
		this.vote = new VoteManager(type, this.num_clients, opts, $("body"), this.callback);
	}
