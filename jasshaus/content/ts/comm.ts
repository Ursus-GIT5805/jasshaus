...
class RTC {
	...
	constructor(config: RTCConfiguration, ice_callback?: (ice: RTCIceCandidate) => void) {
		this.stream = new MediaStream();
		this.pc = new RTCPeerConnection(config);

		this.pc.ontrack = (e) =>
			e.streams[0].getTracks().forEach((track) => this.stream.addTrack(track));
...
export class Client<Data> {
	...
	constructor(name: string, player_id: PlayerID) {
		this.name = name;
...
class LocalRTC {
	...
	constructor(stream: MediaStream, config: RTCConfiguration, ice_callback?: ICECallback) {
		this.stream = stream;
		this.config = config;
...
export class PeerHandler<Data> {
	...
	async init_rtc(turnName: string = "", turnPass: string = "", ice_callback?: ICECallback) {
	...
	setupRTC(id: ClientID) {
		...
		if (rtc && client) {
			...
			rtc.stream.getTracks().forEach((track) => client.rtc?.pc.addTrack(track, rtc.stream));
		}
	}

	// ---

	async rtc_onstart(client_id: ClientID): Promise<undefined | RTCSessionDescription> {
	...
	async rtc_onicecandidate(client_id: ClientID, candidate: RTCIceCandidate) {
		let pc = this.clients.get(client_id)?.rtc?.pc;
		if (!pc) return undefined;
