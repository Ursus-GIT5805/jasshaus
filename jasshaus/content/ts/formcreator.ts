...
function create_basic_form(form: any, ident?: string): Form {
...
function create_option_form(form: any, ident?: string): Form {
	...
	for (const key in form) {
		...
		let option = $("<option>").text(opt_name).attr("value", key);
...
function create_list_form(form: any, ident?: string): Form {
...
export function createForm(form: any, ident?: string, def?: any): Form {
	...
	if (is_option && is_list)
		throw new Error("Form can't be a list and option form at the same time!");
