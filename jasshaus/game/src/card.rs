...
impl Show {
	...
	pub fn as_cards(&self) -> Vec<Card> {
		match self.row {
			1 => (0..NUM_COLORS as u8)
				.map(|i| Card::new(i, self.number))
				.collect(),
			_ => (0..self.row)
		...
		if cards[1].color != col {
			...
			for i in 0..cards.len() - 1 {
				if cards[i].number != cards[i + 1].number
					|| cards[i].color + 1 != cards[i + 1].color
				{
					return Err(());
				}
			}
...
pub fn extract_shows(cards: Vec<Card>) -> Vec<Show> {
	...
	for card in cards {
		let breaks = card.color != last.color || last.number + 1 != card.number;
